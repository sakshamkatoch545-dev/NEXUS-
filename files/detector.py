...
def full_image_analysis(image: Image.Image) -> dict:
    ...
    # Forensic ensemble raw average (Texture, Color, Frequency, Edge, Portrait, Face, ELA)
    forensic_raw = (
        texture["raw"] * 0.20
        + color["raw"]  * 0.20
        + freq["raw"]   * 0.20
    ...
    # Adjustments
    effective_neural = neural["raw"]
    effective_clip   = clip["raw"]
    if forensic_raw > 0.45:
        effective_neural = max(effective_neural, forensic_raw * 0.7)
        effective_clip   = max(effective_clip, forensic_raw * 0.7)
    ...
    if ft_vit.get("active", False):
        final = (
            ...
            + ft_vit["raw"]    * 25.0
            + effective_clip   * 8.0
            + effective_neural * 7.0
        )
    else:
        final = (
            forensic_raw       * 80.0
            + effective_clip   * 10.0
            + effective_neural * 10.0
        )
