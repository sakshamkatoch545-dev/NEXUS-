...
def _lookup_authenticated_registry(image: Image.Image) -> dict | None:
    try:
        ...
        for sha, item in reg.items():
            ...
            if not target_dhash:
                continue
            if target_dhash.lower() == dhash_val.lower():
                return item
        ...
        # 256-bit dhash: distance <= 24 bits represents >= 90.6% visual resemblance
        if best_match and min_dist <= 24:
            return best_match
    except Exception:
        pass
    return None
...
def full_image_analysis(image: Image.Image) -> dict:
    ...
    elif verdict == "AI-GENERATED" or judge_decision == "AI-GENERATED" or ai_conf >= 0.55:
